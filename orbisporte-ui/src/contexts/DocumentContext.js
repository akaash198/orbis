...
const extractPDFText = async (file) => {
  try {
    ...
    console.log('[PDF] Extracted text length:', normalizedText.length, 'characters');
    console.log('[PDF] First 100 chars:', normalizedText.substring(0, 100));
...
const calculateFileHash = async (file) => {
  try {
    ...
    if (isPDF) {
      console.log('[HASH] PDF detected - using content-based hashing');
      try {
        // Extract text content from PDF
        const pdfText = await extractPDFText(file);

        // Hash the text content (not the binary file)
  ...
  const loadDatabaseFiles = async () => {
    try {
      ...
      if (response.success && response.documents) {
        console.log('[LOAD DB FILES] Found', response.documents.length, 'documents');

        // Convert database documents to file state format
        const dbFiles = response.documents.map((doc, idx) => {
          // Create a virtual File object for the document
