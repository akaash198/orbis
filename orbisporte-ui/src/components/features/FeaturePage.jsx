...
export function FeaturePage({ pageId }) {
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        <div className="space-y-8 lg:col-span-8">
                    ...
                    {HS_CODE_MOCK.map((item) => (
                      <tr key={item.id} className="group hover:bg-[#1C2438]/40 transition-colors">
                        ...
                        <td className="px-6 py-5 text-right">
                          <Button variant="ghost" size="sm" icon={<ArrowRight className="h-4 w-4" />}>Select</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {pageId === 'duty-calculator' && (
            <Card className="p-0">
              <CardHeader className="bg-[#111620]">
                <CardTitle>Landed Cost Estimator</CardTitle>
                <CardDescription>Calculate total landing cost with current market rates and duty regimes.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 px-6 pb-6">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Input label="Origin Country" placeholder="Select origin..." icon={<Globe className="h-4 w-4" />} />
                  <Input label="HS Code" placeholder="e.g. 8471.30.10" icon={<FileSearch className="h-4 w-4" />} />
                  <Input label="Transaction Value (INR)" type="number" placeholder="Enter value..." />
                  <Input label="Currency" placeholder="INR" />
                </div>
                <div className="rounded-2xl border border-[#273047] bg-[#0D1020] p-6">
                  <h4 className="flex items-center gap-2 text-sm font-bold text-[#8B97AE]">
                    <TrendingDown className="h-4 w-4 text-[#5AD49A]" />
                    Estimated Tax Summary
                  </h4>
                  <div className="mt-6 space-y-4">
                    <div className="flex justify-between border-b border-[#1E2638] pb-3">
                      <span className="text-sm text-[#8B97AE]">Basic Customs Duty (BCD)</span>
                      <span className="font-mono font-bold text-[#E2E8F5]">INR 0.00</span>
                    </div>
                    <div className="flex justify-between border-b border-[#1E2638] pb-3">
                      <span className="text-sm text-[#8B97AE]">Social Welfare Surcharge (SWS)</span>
                      <span className="font-mono font-bold text-[#E2E8F5]">INR 0.00</span>
                    </div>
                    <div className="flex justify-between border-b border-[#1E2638] pb-3">
                      <span className="text-sm text-[#8B97AE]">Integrated GST (IGST - 18%)</span>
                      <span className="font-mono font-bold text-[#E2E8F5]">INR 0.00</span>
                    </div>
                    <div className="flex justify-between pt-2">
                      <span className="text-base font-bold text-[#E2E8F5]">Net Tax Payable</span>
                      <span className="text-xl font-bold text-[#E8C84A]">INR 0.00</span>
                    </div>
                  </div>
                </div>
                <Button className="w-full" size="lg" icon={<Calculator className="h-5 w-5" />}>Execute Calculation</Button>
              </CardContent>
            </Card>
          )}

          {(pageId === 'fraud-detection' || pageId === 'compliance') && (
          ...
          {!['hs-codes', 'duty-calculator', 'fraud-detection', 'compliance'].includes(pageId) && (
