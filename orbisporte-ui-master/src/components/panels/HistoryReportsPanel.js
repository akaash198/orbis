...
const getPipelineStages = (doc) => ({
  ...
  extracted:  !!(doc.has_extracted_data || (doc.extracted_data && Object.keys(doc.extracted_data).length > 0)),
...
const resolveStatus = (doc) => {
  ...
  if (doc.has_extracted_data || doc.extracted_data) return 'extracted';
...
const HistoryReportsPanel = () => {
  ...
  const fetchDocuments = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const data = await documentService.getDocumentList(500);
  ...
  const handleViewDocument = async (doc) => {
    ...
    try {
      const full = await documentService.getDocumentById(doc.id);
      setViewDoc(full);
    } catch (_) {
      // Keep lightweight metadata if full fetch fails.
    } finally {
