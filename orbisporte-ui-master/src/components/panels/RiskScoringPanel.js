...
import { m05Service, m07Service } from '../../services/api';
...
const DeleteBtn = styled.button`
  ...
  cursor:pointer;
  transition:all .2s;
  &:hover{background:rgba(239,68,68,.2);border-color:rgba(239,68,68,.5)}
...
export default function RiskScoringPanel() {
  ...
  const handleDeleteFiling = useCallback(async (filingId) => {
    const ok = window.confirm(
      `Delete filing #${filingId} permanently?\n\nThis action will remove the selected record and related risk-score data from the system.`
    );
    if (!ok) return;
    ...
    setError(null);
    try {
      // Permanent delete path in M05 requires recycle-bin state first.
      await m05Service.softDelete(filingId);
