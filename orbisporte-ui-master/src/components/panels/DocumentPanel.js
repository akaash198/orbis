...
import { validateFile } from '../common/DocumentPreviewModal';
...
const DocumentPanel = ({ onExtractedItems, pendingDocument, onClearPending, onNavigate }) => {
        ...
        const pollTaskStatus = async () => {
          try {
            ...
            if (statusResult.success && statusResult.status === 'completed') {
              ...
              console.log('Voice extraction completed:', voiceFile?.name);
  ...
  // Essential fields only - no confidence scores or pipeline metadata
  const ESSENTIAL_FIELDS = new Set([
    ...
    'shipment_address', 'hsn_code', 'goods_description', 'product_description',
    'description', 'quantity', 'unit', 'unit_price', 'currency',
  ...
  const filterEssentialFields = (data) => {
    if (!data || typeof data !== 'object') return {};
    const filtered = {};
    for (const [key, value] of Object.entries(data)) {
      if (ESSENTIAL_FIELDS.has(key)) {
        if (key === 'line_items' || key === 'items') {
          // Filter line items to only essential sub-fields
