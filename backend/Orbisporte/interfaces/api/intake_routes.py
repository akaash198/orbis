...
---------
  ...
  GET  /intake/preview/{id}    Get document file for preview (returns base64 data URL)
  GET  /intake/registry        List all documents in the registry (paginated)
  DELETE /intake/document/{id} Permanently delete a document from registry and data lake
...
@router.get("/preview/{document_id}", summary="Get document file URL for preview")
def get_document_preview(
    ...
    """
    Returns a temporary signed URL or data URL for document preview.
    For S3: generates a presigned URL valid for 1 hour.
    For local: returns a data URL (base64 encoded) for the file.
    """
    from fastapi.responses import JSONResponse
    ...
    lake = get_data_lake()
    file_bytes = lake.retrieve_raw(document_id, entry.original_filename)

    if not file_bytes:
        raise HTTPException(status_code=404, detail="File not found in data lake.")

    content_type = _get_content_type(entry.original_filename, entry.file_type)

    import base64
    data_url = f"data:{content_type};base64,{base64.b64encode(file_bytes).decode('latin-1')}"
...
def _get_content_type(filename: str, file_type: str = None) -> str:
    """Determine content type from filename extension or provided file type."""
    import os
    ...
    if file_type:
        ...
        return type_map_ft.get(file_type.lower(), "application/octet-stream")
