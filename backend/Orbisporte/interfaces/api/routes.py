import logging
from fastapi import APIRouter, UploadFile, File, Depends, HTTPException, Header, BackgroundTasks, Body, Form
...
from Orbisporte.interfaces.api.schemas import (
    ...
    DocumentUploadResponse, DocumentResponse, UserResponse,
...
async def hscode_lookup(
    ...
    """
    HSN Classification Engine — LangGraph pipeline.

    normalize (XLM-RoBERTa) → embed (OpenAI) → retrieve (pgvector / FTS)
    ...
    try:
        ...
        result = _get_hsn_service().classify(
        ...
        logger.error("[hscode_lookup] LangGraph pipeline failed: %s", e)
...
async def qa_endpoint(
    ...
    """Answer questions about documents using AI"""
    try:
