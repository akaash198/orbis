...
class Config:
    ...
    GEMINI_MODEL_FALLBACKS = os.getenv("GEMINI_MODEL_FALLBACKS", "gemini-2.5-flash-lite,gemini-2.5-pro")
...
def gemini_client():
    ...
    Uses Gemini 3.1 Flash for fast, accurate document extraction.
    """
    try:
...
def get_gemini_model_candidates() -> List[str]:
    """
    Return ordered Gemini model candidates for extraction workloads.
    Primary is tuned for high throughput + strong quality; fallbacks improve resiliency.
    """
    models: List[str] = []
    ...
    fallbacks = [m.strip() for m in str(Config.GEMINI_MODEL_FALLBACKS or "").split(",") if m.strip()]
