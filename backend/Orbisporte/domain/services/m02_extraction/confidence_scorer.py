...
LLM scorer: Gemini 3.1 Flash (fast, accurate)
...
by Gemini 3.1 Flash. A valid, well-scanned commercial invoice should routinely
...
# ── Gemini 3.1 Flash confidence scoring prompt ──────────────────────────────────
_GEMINI_CONFIDENCE_PROMPT = """You are a quality-assurance specialist for trade document data extraction.
...
def score_fields_with_gpt(
    ...
    # 2 000 chars ≈ 500 tokens — sufficient for header fields where most
    # values appear; reducing from 4 000 cuts prompt size and API latency.
    ocr_sample = (ocr_text or "")[:2000]
    fields_json = json.dumps(scoreable, indent=2, default=str)

    prompt = (
        _GEMINI_CONFIDENCE_PROMPT
        .replace("{ocr_text}", ocr_sample)
        .replace("{fields_json}", fields_json)
    )

    try:
        from Orbisporte.domain.services.llm_service import get_llm_service
        llm = get_llm_service()
        ...
        for field, value in scoreable.items():
            ...
            if raw_score is not None:
                ...
                # LLM omitted this field — fall back per-field
                scores[field] = _rule_based_score(field, value, {})

        logger.info("[Confidence] Gemini 3.1 Flash scored %d fields", len(scores))
        return scores
