...
class DocumentClassificationService:
    ...
    def classify_document(self, file_path: str) -> str:
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": classification_prompt + "\n\nIMPORTANT: A Commercial Invoice always has 'Invoice No' or 'Invoice Number'. A Bill of Lading always has 'Bill of Lading' or 'B/L'. Do NOT confuse invoices with shipping documents."},
                {
                    "role": "user",
                    "content": [
    ...
    def _normalize_classification(self, classification: str) -> str:
        ...
        # Standard mappings — check more specific terms first
        # IMPORTANT: Check commercial_invoice BEFORE purchase_order and before generic "invoice"
        ...
        # Check purchase_order AFTER all invoice types to avoid misclassification
        elif any(term in classification for term in ["purchase order", "purchase_order", "p.o. no", "po number"]):
            return "purchase_order"
        # Generic "invoice" check should come AFTER more specific types
    ...
    def _parse_json_response(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # FALLBACK: Try to extract document type from plain text response
            # This handles cases where LLM returns just "Commercial Invoice" or "Purchase Order"
            content_lower = content.lower().strip()
            detected_type = self._normalize_classification(content_lower)
            
            self.logger.info(f"Fallback classification: {detected_type}")
            return {
                "document_type": detected_type,
                "primary_document_type": detected_type,
                "fallback_used": True,
            }
        except Exception as e:
