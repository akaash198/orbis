...
class IntakeService:
    ...
    def run_pipeline(
        ...
        try:
            ...
            logger.info("DM-004 done: %s in %.3fs", document_id, time.perf_counter() - t0)
            ...
            # Pre-extract sample text once and reuse in dedup + classification.
            t_sample = time.perf_counter()
            text_sample = language_detector._extract_text_sample(processed_bytes, file_type)
            logger.info(
                "DM-005 sample extracted: %s in %.3fs (chars=%d)",
                document_id, time.perf_counter() - t_sample, len(text_sample or "")
            )

            # ── Deduplication + Classification — run in PARALLEL ─────────
            # These two tasks are independent: dedup needs only the text
            # fingerprint; classification needs the processed bytes + text.
            # Running them concurrently cuts ~1–2 s off every upload.
            t_parallel = time.perf_counter()
            text_for_dedup = (text_sample or "").strip()

            def _run_dedup():
                if len(text_for_dedup) < 20:
                    return False, None, 0.0
                from Orbisporte.domain.services.data_intake.deduplication import check_duplicate
                return check_duplicate(document_id, text_for_dedup)

            def _run_classify():
                result = language_detector.detect_language_and_type(
                    processed_bytes, file_type, source_channel, text_sample=text_sample
                )
                _needs_llm = (
                    result["document_type"] == "unknown"
                    or result["classification_confidence"] < 0.4
                )
                if (
                    _needs_llm
                    and file_type in ("pdf", "jpeg", "jpg", "png", "tiff")
                    and bool((text_sample or "").strip())
                ):
                    result = _llm_classify(processed_bytes, file_type, result)
                return result

            with ThreadPoolExecutor(max_workers=2) as _pool:
                fut_dedup    = _pool.submit(_run_dedup)
                fut_classify = _pool.submit(_run_classify)
                is_dup, dup_of, dup_conf = fut_dedup.result()
                detected = fut_classify.result()

            logger.info(
                "Dedup + classify done in parallel: %s in %.3fs",
                document_id, time.perf_counter() - t_parallel,
            )
