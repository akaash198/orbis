...
risk score per shipment. Drives automated routing to clearance, review queue,
 or investigation. Uses supervised CatBoost scoring with explainability for
 regulatory audit compliance.

SOPs
----
  RISK-001  Pull feature vector from M02 / M03 / M04 / M05 / M06
  RISK-002  Supervised CatBoost scoring (rule-based fallback when labels are insufficient)
...
# Module-level scorer singleton — model loaded/trained once and periodically refreshed
_scorer: Optional[CatBoostRiskScorer] = None


def _get_scorer(db: Session) -> CatBoostRiskScorer:
    global _scorer
    if _scorer is None:
        _scorer = CatBoostRiskScorer(db)
    else:
        _scorer.maybe_retrain(db)
    return _scorer
