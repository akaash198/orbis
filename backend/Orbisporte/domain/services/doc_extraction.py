...
from Orbisporte.infrastructure.get_llm import openai_client, gemini_client, Config
...
class DocumentExtractionService:
    """Service for extracting structured data from documents via GPT-4o-mini.
    ...
    def __init__(self, llm_client=None, pdf_processor: PDFProcessor = None):
        self.logger = logger
        # Use OpenAI GPT-4o-mini by default, optionally allow Gemini when requested.
        if llm_client is None:
            ...
            if provider == "gemini" and Config.GEMINI_API_KEY:
                try:
                    ...
                    self.logger.info("Using Gemini 3.1 Flash for document extraction")
                except Exception as e:
                    self.logger.warning(f"Gemini not available ({e}), falling back to OpenAI")
                    self.llm_client = openai_client()
                    self.use_gemini = False
            else:
                self.llm_client = openai_client()
                self.use_gemini = False
                self.logger.info("Using OpenAI gpt-4o-mini for document extraction")
        else:
    ...
    def _call_llm(self, prompt: str, model: str = None, response_format: str = None, 
                  max_tokens: int = 2048) -> str:
        """
        Call LLM with automatic provider detection (Gemini or OpenAI).
        """
    ...
    def _extract_from_image_file(self, file_path: str, document_type: str, extract_barcodes: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # For image-based extraction, use OpenAI (better multi-modal support)
            if not hasattr(self, 'use_gemini') or not self.use_gemini:
                ...
                # Gemini doesn't support base64 images well, use text-only extraction
                content = self._call_llm(direct_extraction_prompt, max_tokens=4000)
    ...
    def _extract_entire_pdf(self, file_path: str, document_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not hasattr(self, 'use_gemini') or not self.use_gemini:
                ...
                # Gemini: use text-only extraction for scanned docs
                content_text = self._call_llm(prompt, max_tokens=4096)
...
"""
                ...
                # Use _call_llm for text-only extraction (works with both Gemini and OpenAI)
                content = self._call_llm(text_prompt, max_tokens=4096)
...
"""
            ...
            # For image batches, use OpenAI (better multi-modal support)
            if not hasattr(self, 'use_gemini') or not self.use_gemini:
                response = self.llm_client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=messages,
                ...
                # Gemini doesn't handle images well, skip this batch
                self.logger.warning(f"Gemini doesn't support image batches, skipping")
                return None
...
"""
                    ...
                    # Use _call_llm for text extraction (Gemini or OpenAI)
                    content = self._call_llm(text_prompt, max_tokens=4096)
            ...
            # Image extraction requires OpenAI (Gemini doesn't support multi-image well)
            response = self.llm_client.chat.completions.create(
                model="gpt-4o-mini",
...
"""
            ...
            validation = self._parse_json_response(content)
